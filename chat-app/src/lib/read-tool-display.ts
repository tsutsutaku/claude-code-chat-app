/** Read / Glob などツール結果プレビューに使う行数（文字列は行、配列は要素数） */
...
/**
 * `safeParseToolInput` 失敗時の `{ _parseError, raw }` を中身に展開する。
 */
...
/**
 * Claude Code / MCP 由来の入れ子から、Read 対象のパス文字列を探す。
 */
...
export type ReadToolDisplay = {
  /** ヘッダー用（ベース名） */
  fileName: string;
  /** サブ行・title 属性用（可能ならフルパス） */
  pathHint: string;
};

/**
 * Read ツールの input / output から表示用のファイル情報を得る。
 */
...
/**
 * @deprecated readToolDisplay を使う
 */
export function readToolFileLabel(input: unknown): string | null {
  const d = readToolDisplay(input);
  return d?.fileName ?? null;
}

/**
 * Read の本文は先頭 N 行だけに切り詰め（長いファイル対策）。
 */
...
/**
 * Glob の結果を先頭 N 行（または配列なら先頭 N 件）に切り詰める。
 */
